	...
	foreach ( $results as $key => $post ) {
		if (is_woocommerce_activated() && ($post->post_type === 'product' || $post->post_type === 'product_variation') ) {
			...
			$suggestions[] = array(
				...
				'price' => '<del><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">$</span>'.$product->get_price_html().'</span></del><ins><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">$</span>'.$product->get_sale_price().' '.'/'.' '.'Pack </span></ins>',
