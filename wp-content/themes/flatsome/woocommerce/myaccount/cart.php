        ...
        if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
          ...
          <tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
            ...
            <td class="product-price" data-title="<?php esc_attr_e( 'Price', 'woocommerce' ); ?>">
              <?php
				$pr = wc_price($_product->get_price_including_tax(1,$_product->get_price()));
       //$pr contains the complete html tag along with the price so first we get a substring of the html that contains price
        $prs = substr($pr,1,-1);

        //Here we isolate the price from HTMl tag and convert to float value
        $newprice = substr($prs,104,-2);

        $testprice =floatval($newprice) / 0.8;

                //echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item );
				echo '<span style="text-decoration: line-through; color: red; font-size: 12pt">'.'$'.$testprice.'</span>'.' '.$pr.' '.'/'.' '.'Pack'; 
              ?>
            </td>
            ...
            <td class="product-subtotal" data-title="<?php esc_attr_e( 'Total', 'woocommerce' ); ?>">
              <?php
				...
				//echo '$'.$cart_item['line_subtotal'].' '.'/'.' '.'Pack';
              ?>
            </td>
          </tr>
          <?php
...
<?php if(get_theme_mod('cart_sticky_sidebar', 1)) { ?>
